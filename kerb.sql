...
CREATE TABLE public.bays (
    id bigint NOT NULL,
    bay_code character varying(255) NOT NULL,
    status character varying(255) DEFAULT 'available'::character varying NOT NULL,
    created_at timestamp(0) without time zone,
    updated_at timestamp(0) without time zone,
    CONSTRAINT bays_status_check CHECK (((status)::text = ANY ((ARRAY['occupied'::character varying, 'available'::character varying])::text[])))
);
...
ALTER TABLE public.bays_id_seq OWNER TO postgres;
...
CREATE TABLE public.books (
    id bigint NOT NULL,
    start_session timestamp(0) without time zone DEFAULT CURRENT_TIMESTAMP NOT NULL,
    end_session timestamp(0) without time zone,
    bill integer DEFAULT 0 NOT NULL,
    payment character varying(255) DEFAULT 'unpaid'::character varying NOT NULL,
    car_id bigint NOT NULL,
    bay_id bigint NOT NULL,
    created_at timestamp(0) without time zone,
    updated_at timestamp(0) without time zone,
    CONSTRAINT books_payment_check CHECK (((payment)::text = ANY ((ARRAY['paid'::character varying, 'unpaid'::character varying])::text[])))
);
...
COPY public.bays (id, bay_code, status, created_at, updated_at) FROM stdin;
1	85QWv	occupied	\N	\N
2	msS2M	available	\N	\N
3	vFYhj	available	\N	\N
\.
...
COPY public.books (id, start_session, end_session, bill, payment, car_id, bay_id, created_at, updated_at) FROM stdin;
1	2021-10-15 21:00:00	\N	0	unpaid	1	1	\N	\N
\.
...
COPY public.cars (id, license_plate, created_at, updated_at) FROM stdin;
1	tHlVbrgz	\N	\N
\.
...
103	2021_10_15_133931_create_cars_table	1
104	2021_10_15_133938_create_bays_table	1
105	2021_10_15_133943_create_books_table	1
\.
...
SELECT pg_catalog.setval('public.bays_id_seq', 3, true);
...
--
-- Name: books books_bay_id_foreign; Type: FK CONSTRAINT; Schema: public; Owner: postgres
--

ALTER TABLE ONLY public.books
    ADD CONSTRAINT books_bay_id_foreign FOREIGN KEY (bay_id) REFERENCES public.bays(id) ON DELETE CASCADE;


--
-- Name: books books_car_id_foreign; Type: FK CONSTRAINT; Schema: public; Owner: postgres
--

ALTER TABLE ONLY public.books
    ADD CONSTRAINT books_car_id_foreign FOREIGN KEY (car_id) REFERENCES public.cars(id) ON DELETE CASCADE;
